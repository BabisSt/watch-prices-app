...
async function Scrape() {
	try {
		...
		const outputDataPath = path.join(__dirname, 'Watches.csv');

		// Adjusted outputPath to save the file in the watch-prices-app/public directory
		const outputAppPath = path.join(__dirname, '..', 'watch-prices-app', 'public', 'Watches.csv');

		fs.writeFile(outputDataPath, "Title,Img,Url,Price_old,Price_new" + '\n' + csvContent, 'utf8', function (err) {
			if (err) {
				console.log('Some error occurred - file either not saved or corrupted.')
			} else {
				console.log('File saved successfully in the current directory');
			}
		})

		fs.writeFile(outputAppPath, "Title,Img,Url,Price_old,Price_new" + '\n' + csvContent, 'utf8', function (err) {
			if (err) {
				console.log('Some error occurred - file either not saved or corrupted.')
			} else {
				console.log('File saved successfully in the current directory');
